# 用法：
#  只做分割：python3 exp_paddle_v3_tuned_params.py --image debug_sauvola_binary_9.png --outdir out --lang ch --to_pagexml
#  分割+文字：python3 exp_paddle_v3_tuned_params.py --image Page_10.png --outdir out --lang ch --to_pagexml --with_rec
...
# ---------- 防崩潰：過大圖片先縮邊 ----------
def safe_resize(image_path, max_side=1200):  # 增加最大尺寸到1200
...
def detect_boxes(image_path, lang='ch', drop_rec=True):
    ...
    if ocr_instance is None:
        ...
        try:
            ...
            ocr_instance = PaddleOCR(
                ...
                det_db_p_ratio=1.8,  # 增加擴展比例
                use_dilation=True,  # 使用膨脹來分離接近的文本行
    ...
    # 讀取並壓縮圖片
    try:
        img, scale = safe_resize(image_path, max_side=1200)  # 增加最大尺寸
        # 轉換為RGB（確保三通道）
        if img.mode != 'RGB':
            img = img.convert('RGB')
        arr = np.array(img)
    ...
    for line in res[0]:
        ...
        if len(scaled_poly) == 4:
            ...
            print(f"[WARN] 跳過異常文本框，點數: {len(scaled_poly)}")
