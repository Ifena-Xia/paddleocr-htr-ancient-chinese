# 用法：
#  單張圖片：python3 paddle_batch_v4_vertical_filter.py --image 190_ef531_default.jpg --outdir out --lang ch --to_pagexml
#  批量處理：python3 paddle_batch_v4_vertical_filter.py --input_dir images --outdir out --lang ch --to_pagexml
...
def filter_vertical_boxes(boxes, min_height_width_ratio=1.5):
    """
    簡單過濾豎排文本框
    min_height_width_ratio: 最小高寬比，大於這個值才被認為是豎排
    """
    filtered_boxes = []
    ...
    for box in boxes:
        ...
        # 避免除以零
        if width == 0:
            filtered_boxes.append(box)  # 寬度為0，可能是豎排線條
            continue
        ...
        # 如果高寬比大於閾值，認為是豎排文本
        if aspect_ratio > min_height_width_ratio:
            filtered_boxes.append(box)
        else:
            # 額外檢查：古籍中即使單字也可能是豎排
            # 如果高度明顯大於寬度（比如1.2倍），也認為是豎排
            if height > width * 1.2 and height > 20:  # 高度至少20像素
                filtered_boxes.append(box)
    
    print(f"[INFO] 從 {len(boxes)} 個框中過濾出 {len(filtered_boxes)} 個豎排版面")
...
def detect_boxes(image_path, lang='ch', drop_rec=True):
    ...
    # 新增：豎排文本過濾（使用方法1）
    if boxes:
        boxes = filter_vertical_boxes(boxes, min_height_width_ratio=1.8)
...
def process_single_image(image_path, args):
    ...
    try:
        result = detect_boxes(image_path, lang=args.lang, drop_rec=(not args.with_rec))
        
        # 檢查返回值是否有效
        if result is None:
            print(f"[ERROR] detect_boxes 返回了 None，跳過此圖片")
            return
