...
def binarize_image(image_path, max_side=1600, method='adaptive'):
    ...
    # Apply slight Gaussian blur first to reduce noise
    gray = cv2.GaussianBlur(gray, (3, 3), 0)
    
    if method == 'adaptive':
        # Improved adaptive binarization - use larger block size and smaller constant
        binary = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                     cv2.THRESH_BINARY, 21, 5)  # adjust parameters
    elif method == 'otsu':
        # Otsu binarization
        _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    elif method == 'sauvola':
        # Sauvola binarization - suitable for document images
        # Adjust window size and parameters for small text
        thresh = threshold_sauvola(gray, window_size=15, k=0.1)  # reduce window size
        binary = (gray > thresh).astype(np.uint8) * 255
    else:
        binary = gray
    
    # Optional morphological operation to enhance text - use gentler parameters
    kernel = np.ones((1, 1), np.uint8)
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel)
    
    # Save binarization result for debugging
    debug_path = f"debug_{method}_binary.png"  # outputs to current working directory
    cv2.imwrite(debug_path, binary)
    print(f"[DEBUG] Binarization result saved: {debug_path}")
    
    # Convert back to PIL format - ensure it is RGB three channels
    img = Image.fromarray(binary).convert('RGB')  # Important: convert to RGB
    print(f"[INFO] Binarization preprocessing applied (method: {method})")
...
def detect_boxes(image_path, lang='ch', drop_rec=True, preprocess_mode="none"):
    ...
    if ocr_instance is None:
        ...
        try:
            ...
            ocr_instance = PaddleOCR(
                ...
                cls=True,
                # 對於二值化圖像，可能需要調整閾值
                det_db_thresh=0.1,  # 進一步降低檢測閾值
                det_db_box_thresh=0.2,  # 進一步降低框閾值
                det_db_unclip_ratio=1.6,  # 適中擴展比例
                use_dilation=True,
                det_limit_side_len=1600,
                det_db_score_mode='fast',
                # 針對二值化圖像的特殊設置
                det_model_dir=None,  # 使用默認模型
                rec_model_dir=None,
            )
            print("[INFO] PaddleOCR初始化成功（二值化圖像優化版）")
            signal.alarm(0)
    ...
    try:
        ...
        debug_img.save("debug_preprocessed.png")
        print("[DEBUG] 預處理後圖像已保存: debug_preprocessed.png")
    ...
    try:
        ...
        detected_count = len(res[0]) if res and res[0] else 0
        print(f"[INFO] OCR處理完成，檢測到 {detected_count} 個文本框")
    ...
    print(f"[INFO] 有效文本框數量: {valid_boxes}")
    return boxes, scale

# 過濾掉可能不是文字的區域
def filter_boxes(boxes, min_area=20, max_aspect_ratio=15, min_text_confidence=0.05):
    """
    放寬過濾條件以適應小文字
    """
    filtered_boxes = []
    ...
    for box in boxes:
        ...
        if (area >= min_area and 
            ...
            print(f"[DEBUG] 過濾掉框: 面積={area}, 寬高比={aspect_ratio:.2f}, 置信度={box.get('score', 1.0):.3f}")
...
def main():
    ...
    try:
        ...
        if not boxes:
            ...
            # 嘗試無預處理模式作為備用
            print("[INFO] 嘗試無預處理模式作為備用...")
            boxes, scale = detect_boxes(args.image, lang=args.lang, 
                                       drop_rec=(not args.with_rec), 
                                       preprocess_mode="none")
            if boxes:
                print(f"[INFO] 備用模式偵測到 {len(boxes)} 個文本框")
            else:
                return
